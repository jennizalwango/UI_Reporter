...
class APITestCase(BaseTestCase):
    ...
    def test_get_one_incident(self):
        with self.client:
            self.client.post('/api/v1/register', data=json.dumps(self.create_user),content_type='application/json')
            response = self.client.post('/api/v1/incident', data=json.dumps(self.create_incident), content_type='application/json')
    ...
    def test_all_feilds_are_missing(self):
        with self.client:
            response = self.client.post('/api/v1/incident', data=json.dumps(self.required_feilds),content_type='application/json')
            self.assertEqual(response.status_code, 400)
            self.assertIn("Please provide valid fields", str(response.data))  
    ...
    def test_update_location(self):
        with self.client:
            response = self.client.post('/api/v1/register', data=json.dumps(self.create_user),content_type='application/json')
            response = self.client.post('/api/v1/incident', data=json.dumps(self.create_incident),content_type='application/json')
            response =self.client.patch('/api/v1/incident/1/location',data=json.dumps({'location':'location'}), content_type='application/json')
            self.assertIn("Updated incident record", str(response.data))
            self.assertEqual(response.status_code, 200)

    def test_location_not_found(self):
        with self.client:
            response =self.client.post('/api/v1/register',data=json.dumps(self.create_user))
            response =self.client.post('/api/v1/incident',data=json.dumps(self.create_incident), content_type='application/json')
            response =self.client.patch('/api/v1/incident/1/location',data=json.dumps({}),content_type='application/json')
            self.assertEqual(response.status_code, 400)
    ...
    def test_update_comment(self):
        with self.client:
            ...
            response =self.client.post('/api/v1/register',data=json.dumps(self.create_user))
            response =self.client.post('/api/v1/incident',data=json.dumps(self.create_incident), content_type='application/json')
            response = self.client.patch('/api/v1/incident/1/comment',data=json.dumps(incident_data), content_type='application/json')
            self.assertEqual(response.status_code, 200)

    def test_delete_a_specific_incident(self):
        with self.client:
            self.client.post('/api/v1/incident', data=json.dumps(self.create_incident))
            response = self.client.delete('/api/v1/incident/1', content_type='application/json')
            self.assertEqual(response.status_code, 400)
